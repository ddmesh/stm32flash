/*
  ...
  Copyright (C) 2012-2016 Tormod Volden <debian.tormod@gmail.com>
  Copyright (C) 2013-2016 Antonio Borneo <borneo.antonio@gmail.com>
  Copyright (C) 2021 Renaud Fivet <renaud.fivet@gmail.com>
	Copyright (C) 2023 Stephan Enderlein <stephan@freifunk-dresden.de>
...
//ddmesh
extern const stm32_dev_t stm32_devices[];
extern const stm32_dev_t gd32_devices[];
...
int main(int argc, char* argv[]) {
	...
	stm = stm32_init(port, init_flag, option_gd32 ? gd32_devices : stm32_devices);
	if (!stm)
		goto close;
...
//ddmesh
			fprintf(diag, "Erasing memory (start page:%u count:%u)\n", first_page, num_pages);
			s_err = stm32_erase_memory(stm, first_page, num_pages);
			if (s_err != STM32_ERR_OK) {
				fprintf(stderr, "Failed to erase memory\n");
				goto close;
...
int parse_options(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "a:b:m:r:w:e:vn:g:jkfcChuos:S:F:i:RG")) != -1) {
		switch(c) {
			case 'a':
...
//ddmesh
		"	-G		Select GigaDevice (default STM32).\n"
		"\n"
		"GPIO sequence:\n"
...
//ddmesh
		"\n	GigaDevices:\n\n"
		"	flash full file:\n"
		"		  %s -G -b 115200 -w filename /dev/ttyUSB0\n\n"
		"	flash only first n bytes to specific start address:\n"
		"		  %s -G -b 115200 -w filename -S 0x8010000:10000 /dev/ttyUSB0\n"
		"\n",
		name,
		name,
		name,
